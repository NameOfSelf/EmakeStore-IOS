    ...
    switch message.messageStatus {
      ...
      case .mediaDownloadFail:
        ...
        if message.isOutGoing {
            ...
            let part = message.fromUser.clientID().components(separatedBy: "/")
            if part.count == 3{
                self.nameLabel.text = String(format: "小二 %@", arguments: [message.fromUser.displayName()])
            }else{
                self.nameLabel.text = String(format: "%@", arguments: [message.fromUser.displayName()])
            }
        }
        if !message.isOutGoing {
            let part = message.fromUser.clientID().components(separatedBy: "/")
            if part.count == 3{
                self.avatarImage.sd_setImage(with: URL(string: message.fromUser.Avatar()), placeholderImage: UIImage(named: "guanfangkefu"))
            }else{
                self.avatarImage.sd_setImage(with: URL(string: message.fromUser.Avatar()), placeholderImage: UIImage(named: "kefu_logo"))
            }
        }else{
            self.avatarImage.image = UIImage(named: message.fromUser.Avatar())
        }
